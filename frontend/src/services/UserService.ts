...
import apiClient, { publicApi, authUtils } from '../api/client'
import { 
  transformSignUpData, 
  transformSignInData, 
...
export const signup = (data: bookcarsTypes.SignUpPayload): Promise<number> => {
  try {
    ...
    return publicApi
      .post('/public/auth/register', transformedData)
      .then((res) => res.status)
      .catch((error) => {
...
export const resend = (email?: string, reset = false): Promise<number> => {
  if (reset && email) {
    // Use new API for password reset
    try {
      return publicApi
        .post('/public/auth/forgot-password', { email })
        .then((res) => res.status)
        .catch((error) => {
          // Fallback to legacy API
          return axiosInstance
            .post(`/api/resend/${env.APP_TYPE}/${encodeURIComponent(email)}/${reset}`)
            .then((res) => res.status);
        });
    } catch (error: any) {
  ...
  // For non-reset cases, use legacy API
  return axiosInstance
    .post(`/api/resend/${env.APP_TYPE}/${encodeURIComponent(email || '')}/${reset}`)
    .then((res) => res.status);
}
...
export const signin = (data: bookcarsTypes.SignInPayload): Promise<{ status: number, data: bookcarsTypes.User }> => {
  try {
    const transformedData = transformSignInData(data);
    return publicApi
      .post('/auth/login', transformedData)
      .then((res) => {
        const { token, user } = extractAuthToken(res.data);
        
        // Store JWT token
        authUtils.setToken(token);
        
        // Store user data in legacy format for compatibility
...
export const signout = async (redirect = true, redirectSignin = false) => {
  ...
  authUtils.removeToken() // Remove JWT token
  deleteAllCookies()
  ...
  try {
    ...
    try {
      ...
      console.error('Sign out failed:', legacyError);
    }
  }
...
export const validateAccessToken = (): Promise<number> => {
  ...
  if (authUtils.isAuthenticated()) {
    return apiClient
      ...
      .then((res) => res.status)
      .catch((error) => {
        // If JWT validation fails, try legacy
        return axiosInstance
...
export const changePassword = (data: bookcarsTypes.ChangePasswordPayload): Promise<number> => {
  // Try new API first if authenticated with JWT
  if (authUtils.isAuthenticated()) {
    return apiClient
...
export const sendEmail = (payload: any): Promise<number> =>
  axiosInstance
    .post(
      '/api/send-email',
      payload,
 ...
 * @returns {Promise<number>}
 */
export const resetPassword = (token: string, newPassword: string): Promise<number> => {
  try {
    return publicApi
 ...
 * @returns {Promise<number>}
 */
export const forgotPassword = (email: string): Promise<number> => {
  try {
    return publicApi
      .post('/public/auth/forgot-password', { email })
      .then((res) => res.status)
