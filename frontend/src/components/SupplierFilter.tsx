...
const SupplierFilter = ({
  ...
  const handleCheckSupplierChange = (
      ...
      if (checkedSuppliers.length === 0) {
        onChange(
          bookcarsHelper.clone(
            bookcarsHelper.flattenSuppliers(supliesFromProps)
          )
        );
      } else {
    ...
    (suppliers.length > 1 && (
      <Accordion
        ...
        offsetHeight={Math.floor(
          (suppliers.length / 2) * env.SUPPLIER_IMAGE_HEIGHT
        )}
        ...
        <ul className="supplier-list">
          {suppliers.map((supplier, index) => (
            <li key={supplier._id}>
              ...
              {!!supplier.carCount && (
                <span className="car-count">{`(${supplier.carCount})`}</span>
              )}
            </li>
          ))}
        </ul>
