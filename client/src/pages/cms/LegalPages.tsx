...
// Define status options for legal pages
...
const LegalPages = () => {
  ...
  const loadPages = async () => {
    ...
    clearError()
    try {
      const response = await cmsAPI.getLegalPages({
      ...
      const errorMessage = err.response?.data?.message || err.message || 'Failed to load legal pages'
  ...
  const loadStats = async () => {
    try {
      ...
      const legalPages = response.recentPages?.filter(p => p.status === 'published') || []
      setStats({
        total_documents: response.byType.legal || 0,
        published_documents: response.byStatus.published || 0,
        draft_documents: response.byStatus.draft || 0,
        archived_documents: response.byStatus.archived || 0,
        documents_in_menu: legalPages.length,
        last_updated: legalPages[0]?.createdAt || new Date().toISOString()
      })
    } catch (err: any) {
  ...
  const handlePublishPage = async (page: CmsPage) => {
    try {
      ...
      showNotification(`Legal document "${page.title}" published successfully`, 'success')
      loadPages()
      loadStats()
    } catch (err: any) {
      const errorMessage = err.response?.data?.message || err.message || 'Failed to publish legal document'
      showNotification(errorMessage, 'error')
    }
  }
  ...
  const handleUnpublishPage = async (page: CmsPage) => {
    try {
      ...
      showNotification(`Legal document "${page.title}" unpublished successfully`, 'success')
      loadPages()
      loadStats()
    } catch (err: any) {
      const errorMessage = err.response?.data?.message || err.message || 'Failed to unpublish legal document'
      showNotification(errorMessage, 'error')
    }
  }
  ...
  const handleSubmitAddPage = async () => {
    ...
    clearError()
    try {
      const createdPage = await cmsAPI.createLegalPage(newPage)
      showNotification(`Legal document "${createdPage.title}" created successfully`, 'success')
      setOpenAddDialog(false)
      loadPages()
      loadStats()
    } catch (err: any) {
      const errorMessage = err.response?.data?.message || err.message || 'Failed to create legal document'
      setError(errorMessage)
      showNotification(errorMessage, 'error')
    } finally {
  ...
  const handleSubmitEditPage = async () => {
    ...
    try {
      ...
      showNotification(`Legal document "${updatedPage.title}" updated successfully`, 'success')
      setOpenEditDialog(false)
      setSelectedPage(null)
      loadPages()
      loadStats()
    } catch (err: any) {
      const errorMessage = err.response?.data?.message || err.message || 'Failed to update legal document'
      setError(errorMessage)
      showNotification(errorMessage, 'error')
    } finally {
      setFormLoading(false)
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      showNotification(`Legal document "${selectedPage.title}" deleted successfully`, 'success')
      setOpenDeleteDialog(false)
      setSelectedPage(null)
      loadPages()
      loadStats()
    } catch (err: any) {
      const errorMessage = err.response?.data?.message || err.message || 'Failed to delete legal document'
      showNotification(errorMessage, 'error')
    }
  }
    ...
    <Box>
      ...
      <TableContainer component={Paper}>
        <Table>
          <TableHead>
            <TableRow>
              <TableCell>Legal Document</TableCell>
      ...
      <Dialog open={openAddDialog} onClose={() => setOpenAddDialog(false)} maxWidth="md" fullWidth>
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={12}>
                <TextField
                  ...
                  rows={10}
                  required
                  helperText="HTML content of the legal document"
                />
              </Grid>
              <Grid item xs={12} sm={8}>
                <TextField
                  ...
                  helperText="Optional featured image for the document"
                />
              </Grid>
        ...
        <DialogActions>
          ...
          >
            {formLoading ? <CircularProgress size={20} /> : 'Add Legal Document'}
          </Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={openEditDialog} onClose={() => setOpenEditDialog(false)} maxWidth="md" fullWidth>
        ...
        <DialogActions>
          ...
          >
            {formLoading ? <CircularProgress size={20} /> : 'Update Legal Document'}
          </Button>
        </DialogActions>
      </Dialog>

      {/* Delete Legal Document Dialog */}
      <ConfirmDialog
        open={openDeleteDialog}
        onClose={() => setOpenDeleteDialog(false)}
        onConfirm={handleConfirmDelete}
        title="Delete Legal Document"
        message={`Are you sure you want to delete the legal document "${selectedPage?.title}"? This action cannot be undone and may affect compliance.`}
        confirmText="Delete"
        severity="error"
      />

      {/* View Legal Document Details Dialog */}
      <Dialog open={openViewDialog} onClose={() => setOpenViewDialog(false)} maxWidth="md" fullWidth>
        <DialogTitle>
          <Box display="flex" alignItems="center" gap={2}>
            <GavelIcon />
            <Typography variant="h6">
              Legal Document Details: {selectedPage?.title}
            </Typography>
          </Box>
        </DialogTitle>
        <DialogContent>
                  ...
                  <Paper sx={{ p: 2, bgcolor: 'grey.50' }}>
                    ...
                    >
                      <Typography variant="body2" style={{ whiteSpace: 'pre-wrap' }}>
                        {selectedPage.content.substring(0, 800)}
                        {selectedPage.content.length > 800 && '...'}
                      </Typography>
                    </Box>
                  </Paper>
                </Grid>
