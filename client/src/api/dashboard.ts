...
export const analyticsAPI = {
  ...
  getBookingAnalytics: async (period: string = '30d'): Promise<BookingAnalytics> => {
    const response = await apiClient.get(`/api/v1/analytics/bookings?period=${period}`)
    return response.data
  },

  // Revenue Analytics
  getRevenueAnalytics: async (period: string = '12m'): Promise<RevenueAnalytics> => {
    const response = await apiClient.get(`/api/v1/analytics/revenue?period=${period}`)
    return response.data
  },

  // User Analytics
  getUserAnalytics: async (): Promise<UserAnalytics> => {
    const response = await apiClient.get('/api/v1/analytics/users')
    return response.data
  },

  // Driver Performance
  getDriverPerformance: async (): Promise<DriverPerformance> => {
    const response = await apiClient.get('/api/v1/analytics/drivers')
    return response.data
  },

  // Company Analytics
  getCompanyAnalytics: async (): Promise<CompanyAnalytics> => {
    const response = await apiClient.get('/api/v1/analytics/companies')
    return response.data
  },

  // Coupon Analytics
  getCouponAnalytics: async (): Promise<CouponAnalytics> => {
    const response = await apiClient.get('/api/v1/analytics/coupons')
    return response.data
  },

  // Operational Metrics
  getOperationalMetrics: async (): Promise<OperationalMetrics> => {
    const response = await apiClient.get('/api/v1/analytics/operations')
    return response.data
  },
  ...
  getFinancialSummary: async (period: string = '30d') => {
    const response = await apiClient.get(`/api/v1/analytics/financial-summary?period=${period}`)
    return response.data
  },

  // Performance summary
  getPerformanceSummary: async () => {
    const response = await apiClient.get('/api/v1/analytics/performance-summary')
    return response.data
  },

  // Growth metrics
  getGrowthMetrics: async (period: string = '12m') => {
    const response = await apiClient.get(`/api/v1/analytics/growth?period=${period}`)
    return response.data
  },

  // Export analytics
  exportAnalytics: async (format: string = 'json', period: string = '30d') => {
    const response = await apiClient.get(`/api/v1/analytics/export?format=${format}&period=${period}`)
    return response.data
  },
}
